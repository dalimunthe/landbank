...
def file_directory(files):
    files_array = files.split("\\")
    files_name = files_array[-1]
    file_nm = files_name.split(".")
    files_ext = "." + file_nm[-1]
    files_name = files_name.replace(files_ext, "")
    files_array = files_array[:-1]
    directory_files = "\\".join(files_array) + "\\"

    return [directory_files, files_name, files_ext]
        ...
        for idx, ax in enumerate(dum_row):
            ...
            if ax[3] in x_dup and ax[4] in y_dup:
                final_arr.append([ax[0], ax[1],
                                  ax[2], ax[3],
                                  ax[4], ax[5],
                                  distances, "SHARED", self.legal_type, "", idx + 1])
            else:
                final_arr.append([ax[0], ax[1],
                                  ax[2], ax[3],
                                  ax[4], ax[5],
...
class arrayToPolygon(object):
    ...
    def create_polygon(self):
        ...
        for key, rows in groupby(self.array_polygon, lambda x: (x[11])):
            ...
            arcpy.AddField_management(self.out_put_path + key + ".shp", "JobID", "LONG", 9, "", "", "JobID", "NULLABLE",
                                      "REQUIRED")
            ...
            pt = arcpy.CreateFeatureclass_management(self.out_put_path, key + "_pt.shp", "POINT", template, has_m,
                                                     has_z, spatial_reference)
            arcpy.AddField_management(self.out_put_path + key + "_pt.shp", "JobID", "LONG", 9, "", "", "JobID",
                                      "NULLABLE",
                                      "REQUIRED")
...
class Toolbox(object):
    def __init__(self):
        ...
        .pyt file)."""
        self.label = "Landbank Tools"
        self.alias = "Landbank"

        # List of tool classes associated with this toolbox
        self.tools = [polygonToExcel, excelToPolygon]


class polygonToExcel(object):
    def __init__(self):
        """Define the tool (tool name is the name of the class)."""
        self.label = "Polygon To Excel"
        self.description = "Convert Polygon To Excel"
        self.canRunInBackground = False

    def getParameterInfo(self):
        """Define parameter definitions"""
        feature_class = arcpy.Parameter(
            displayName="Landbank Polygon",
            name="feature_class",
            datatype="GPFeatureLayer",
            parameterType="Required",
            direction="Input"
        )
        feature_class.filter.list = ["Polygon"]
    ...
    def execute(self, parameters, messages):
        ...
        legal_type = ''
        nama_pt = ''
        out_path = parameters[1].valueAsText
        nf = file_directory(feature_class)
        nama_file = nf[1]
        conv = convertVertices(feature_class=feature_class,
...
class excelToPolygon(object):
    def __init__(self):
        """Define the tool (tool name is the name of the class)."""
        self.label = "Excel To Polygon"
        self.description = "Convert Excel To Polygon"
        self.canRunInBackground = False

    def getParameterInfo(self):
        """Define parameter definitions"""
        excel_file = arcpy.Parameter(
            displayName="Landbank Excel",
            name="excel_file",
            datatype="DEFile",
            parameterType="Required",
            direction="Input"
        ...
        excel_file.filter.list = ['xls', 'xlsx']
    ...
    def execute(self, parameters, messages):
        """The source code of the tool."""
        excel_file = parameters[0].valueAsText
        out_put_path = parameters[1].valueAsText

        excel = convertExcel(excel_file=excel_file)
        arcpy.AddMessage('Read Excel File')
        ...
        arcpy.AddMessage('Create Polygon')
        ress = cp.create_polygon()
        arcpy.AddMessage('Create Report')
        pdf = createReport(out_path=out_put_path, data_array=data, info_array=ress, detil_array=info)
        pdf.build_earthwork_resume()
        return
